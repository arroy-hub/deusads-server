...
// The SDK is also called from browsers (WebGL builds hosted on itch.io and
// elsewhere). Requests authenticate by the X-DeusADS-Key header, never by
// cookies, so any origin may call the API: the key is what gates access.
export const CORS_HEADERS = {
  "access-control-allow-origin": "*",
  "access-control-allow-methods": "GET, POST, OPTIONS",
  "access-control-allow-headers": "Content-Type, X-DeusADS-Key",
...
/** Answers the browser's CORS preflight for the SDK routes. */
...
export async function authenticateGame(request) {
  ...
  const lookup = () =>
    db.from("games").select("id, owner_id, name").eq("api_key", apiKey.trim()).maybeSingle();

  let { data: game, error } = await lookup();
  const missingSchema = (e) => /relation .* does not exist|schema cache/i.test(e?.message ?? "");

  if (error && !missingSchema(error)) {
    // A cold function or a brief database hiccup fails once and then recovers.
    // The SDK asks for the manifest only at startup, so one retry here is what
    // keeps a first-time player from seeing only fallbacks.
    console.warn("[DeusADS] auth lookup failed, retrying:", error.message);
    await new Promise((resolve) => setTimeout(resolve, 300));
    ({ data: game, error } = await lookup());
  }

  if (error) {
    // A missing table means the schema was never run; say so rather than
    // reporting a generic outage.
    const message = missingSchema(error)
...
const warnedOutdated = new Set();
export function warnSchemaOutdated(where, migration = "0002_placement_sync_and_events.sql") {
  if (warnedOutdated.has(migration)) return;
  warnedOutdated.add(migration);
  console.warn(
    `[DeusADS] ${where}: database schema is behind the code. ` +
      `Run supabase/migrations/${migration} in the Supabase SQL editor.`
  );
}
